...
Stack **real** del repositorio (jun 2026). Las tecnologías aspiracionales del documento original (`_docs/02-TECH_STACK.md`) que aún no están en el proyecto se listan al final.
...
1. **I/O y parseo pesado en Rust** — no en el hilo principal de React.
2. **IPC tipado** — contratos en `src/lib/types/` y `src-tauri/src/parser/types.rs`.
3. **i18n** — sin strings hardcodeados en UI.
4. **Licencias permisivas** — MIT/Apache (p. ej. Excalidraw sí, Tldraw BSL no).
5. **Offline-first** — sin dependencias de red para el núcleo.

Ver convenciones de desarrollo en `_docs/07-GUIDELINES-LLMs.md` (pendiente de copia a `_docs2/`).
