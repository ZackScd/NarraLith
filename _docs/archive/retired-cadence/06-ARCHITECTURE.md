...
`.taurignore` excluye `_docs/` y `_docs/` del binario.
...
**Eventos:** prefijo `obs.ui.*` (catálogo en [`specs/plans/_archive/Fase C/OBS-002-ui-render-audit-log.md`](specs/plans/Fase%20C/OBS-002-ui-render-audit-log.md) §3). **Nunca** usar `audit.info("ui", …)` — siempre `renderAudit.ui()`.
...
**Eventos:** prefijo `obs.action.*` (catálogo en [`specs/plans/_archive/Fase D/OBS-003-action-audit-log.md`](specs/plans/Fase%20D/OBS-003-action-audit-log.md) §4). **Nunca** usar `audit.info` ni `renderAudit.ui` para acciones.
